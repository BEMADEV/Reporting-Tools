...
namespace com.bemaservices.ReportingTools.Jobs
{
    ...
    public class CalculateReportingFields : IJob
    {
        ...
        public virtual void Execute( IJobExecutionContext context )
        {
            ...
            while ( results.Count > 0 )
            {
                ...
                foreach ( var result in results )
                {
                    ...
                    SaveAttributeSql( familyHeadOfHouseholdAttribute, result.Id, result.FamilyHeadofHousehold, commandTimeout, false, true );
                    ...
                    SaveAttributeSql( givingUnitHeadOfHouseholdAttribute, result.Id, result.GivingUnitHeadofHousehold, commandTimeout, false, true );
        ...
        private static bool SaveAttributeSql( AttributeCache attribute, int personId, string value, int commandTimeout, bool setDateTime = false, bool setBoolean = false )
        {
            if ( attribute == null )
            {
                string message = String.Format( "Error Finding Attribute for Person with ID {1}", personId );
            ...
            {
                string message = "Error Finding PersonId";
                ExceptionLogService.LogException( new Exception( message ) );
                return false;
            }
            ...
            try
            {
                    var query = string.Format( @"
                       ...
                       attribute.Id,
                       value.Replace( "'", "''" ),
                       setDateTime ? "GetDate()" : "NULL",
                       setBoolean ? value.AsBoolean().Bit().ToString() : "NULL"
                   );
